...
const sizes = {vedetjes: 3, speelclub: 3, rakwi: 3, tito: 2, keti: 2, aspi: 2}

function geneticSolver(groups, ofSize, forRounds, forbiddenPairs=[], wantedPairs = [], onProgress) {
    ...
    const weights = _.range(totalSize).map(() => _.range(totalSize).fill(Infinity/2))
    console.log(totalSize)
    // Fill some initial restrictions
    forbiddenPairs.forEach(([a, b]) => {
        console.log(weights[a,b])
        if (a >= totalSize || b >= totalSize) return
        weights[a][b] = weights[b][a] = Infinity
    })

    wantedPairs.forEach(([a, b]) => {
        console.log(weights[a,b])
        if (a >= totalSize || b >= totalSize) return
        weights[a][b] = weights[b][a] = 0
    })
